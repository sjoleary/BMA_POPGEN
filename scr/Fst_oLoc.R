...
gen_o <- genind.rem.loci(gen, removeloc)
...
tidy <- tidy_genomic_data(data = gen_o, filename = NULL, parallel = 65)
...
write_delim(df, "results/estuaries_oLoc.globalfst.ci", delim = "\t")
...
write_delim(results, "results/estuary_oLoc.globalfst.pval", delim = "\t")
...
tidy <- tidy_genomic_data(data = gen_o, filename = NULL, parallel = 10)
...
write_delim(df, "results/ocean_oLoc.fst.ci", delim = "\t")
...
write_delim(temp, "results/ocean_oLoc.fst.WC84", delim = "\t")
...
write_delim(fst_perm, "results/ocean_oLoc.fst.perm", delim = "\t")
...
write_delim(pval, "results/ocean_oLoc.fst.pval", delim = "\t")
...
write_delim(df, "results/region_oLoc.fst.ci", delim = "\t")
...
write_delim(temp, "results/region_oLoc.fst", delim = "\t")
...
write_delim(fst_perm, "results/region_oLoc.fst.perm", delim = "\t")
...
write_delim(pval, "results/region_oLoc.fst.pval", delim = "\t")
...
# pairwise Fst and CIs ====
tidy <- tidy_genomic_data(data = gen_o, filename = NULL, parallel = 20)
...
write_delim(df, "results/estuaries_oLoc.fst.ci", delim = "\t")
...
gen_o <- as.data.frame(mat.obs) %>%
  rownames_to_column("GRP1") %>%
  gather(key = "GRP2", value = "obsFst", 2:(n+1)) %>%
  filter(GRP1 != GRP2)

write_delim(gen_o, "results/estuary_oLoc.fst.WC84", delim = "\t")
...
write_delim(fst_perm, "results/estuary_oLoc.fst.perm", delim = "\t")
...
write_delim(pval, "results/estuary_oLoc.fst.pval", delim = "\t")
