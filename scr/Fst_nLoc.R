...
tidy <- tidy_genomic_data(data = gen_n, filename = NULL, parallel = 65)
...
write_delim(df, "results/estuaries_nLoc.globalfst.ci", delim = "\t")

# global fst and permuted p-values ==== 

dat <- genind2hierfstat(gen_n)
dat_rand <- dat
...
write_delim(results, "results/estuary_nLoc.globalfst.pval", delim = "\t")
...
tidy <- tidy_genomic_data(data = gen_n, filename = NULL, parallel = 10)
...
write_delim(df, "results/ocean_nLoc.fst.ci", delim = "\t")
...
write_delim(temp, "results/ocean_nLoc.fst.WC84", delim = "\t")
...
write_delim(fst_perm, "results/ocean_nLoc.fst.perm", delim = "\t")
...
write_delim(pval, "results/ocean_nLoc.fst.pval", delim = "\t")
...
write_delim(df, "results/region_nLoc.fst.ci", delim = "\t")
...
write_delim(temp, "results/region_nLoc.fst", delim = "\t")
...
write_delim(fst_perm, "results/region_nLoc.fst.perm", delim = "\t")
...
write_delim(pval, "results/region_nLoc.fst.pval", delim = "\t")
...
# pairwise Fst and CIs ====
tidy <- tidy_genomic_data(data = gen_n, filename = NULL, parallel = 20)
...
write_delim(df, "results/estuaries_nLoc.fst.ci", delim = "\t")
...
gen_n <- as.data.frame(mat.obs) %>%
  rownames_to_column("GRP1") %>%
  gather(key = "GRP2", value = "obsFst", 2:(n+1)) %>%
  filter(GRP1 != GRP2)

write_delim(gen_n, "results/estuary_nLoc.fst.WC84", delim = "\t")
...
write_delim(fst_perm, "results/estuary_nLoc.fst.perm", delim = "\t")
...
write_delim(pval, "results/estuary_nLoc.fst.pval", delim = "\t")
