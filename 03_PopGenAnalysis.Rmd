...
col_clust <- c("#442288", "#6CA2EA", "#B5D33D", "#FED23F", "#EB7D5B")
...
kable(
  count(strata, POP),
  caption = "Number of sample per putative population, Florida Atlantic (ATL), South and North Florida (FLGS, FLGN), Alabama (MB), Mississippi (MISS), Louisiana (CS, LA), Texas (CC), and Campeche (SG)."
)
...
```{r fig.cap="Fig 1: AIC for data clustered into K = 1 - 10 groups using k-means clustering.", fig.height=4, fig.width=7}
...
het <- read_delim("results/gendiv.locstats", delim = "\t") %>%
  ...
  mutate(GRP = ordered(GRP, levels = pops),
         REGION = case_when(GRP %in% c("ATL") ~ "SWATL",
                            GRP %in% c("CAMP") ~ "SGULF",
...
write_delim(tmp, "results/allelrich_est_matrix.wilcox")

tmp <- results %>%
  select(-stat) %>%
  pivot_wider(names_from = "pop2", values_from = "p.value")

write_delim(tmp, "results/allelrich_est_matrix.pval.wilcox")
...
write_delim(results, "results/evenness_est.wilcox", delim = "\t")

tmp <- results %>%
  select(-`p.value`) %>%
  pivot_wider(names_from = "pop2", values_from = "stat")

write_delim(tmp, "results/evenness_est_matrix.wilcox")

tmp <- results %>%
  select(-stat) %>%
  pivot_wider(names_from = "pop2", values_from = "p.value")

write_delim(tmp, "results/evenness_est_matrix.pval.wilcox")
...
write_delim(tmp, "results/allelrich_est_matrix.o.wilcox")

tmp <- results %>%
  select(-stat) %>%
  pivot_wider(names_from = "pop2", values_from = "p.value")

write_delim(tmp, "results/allelrich_est_matrix.pval.o.wilcox")
...
write_delim(results, "results/evenness_est.o.wilcox", delim = "\t")

tmp <- results %>%
  select(-`p.value`) %>%
  pivot_wider(names_from = "pop2", values_from = "stat")

write_delim(tmp, "results/evenness_est_matrix.o.wilcox")

tmp <- results %>%
  select(-stat) %>%
  pivot_wider(names_from = "pop2", values_from = "p.value")

write_delim(tmp, "results/evenness_est_matrix.pval.o.wilcox")
...
write_delim(tmp, "results/exphet_est_matrix.n.wilcox")

tmp <- results %>%
  select(-stat) %>%
  pivot_wider(names_from = "pop2", values_from = "p.value")

write_delim(tmp, "results/exphet_est_matrix.pval.n.wilcox")
...
write_delim(tmp, "results/allelrich_est_matrix.n.wilcox")

tmp <- results %>%
  select(-stat) %>%
  pivot_wider(names_from = "pop2", values_from = "p.value")

write_delim(tmp, "results/allelrich_est_matrix.pval.n.wilcox")
...
write_delim(results, "results/evenness_est.n.wilcox", delim = "\t")

tmp <- results %>%
  select(-`p.value`) %>%
  pivot_wider(names_from = "pop2", values_from = "stat")

write_delim(tmp, "results/evenness_est_matrix.n.wilcox")

tmp <- results %>%
  select(-stat) %>%
  pivot_wider(names_from = "pop2", values_from = "p.value")

write_delim(tmp, "results/evenness_est_matrix.pval.n.wilcox")
